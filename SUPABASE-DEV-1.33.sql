...
-- DEV 1.33: shared CPU rooms
alter table public.rooms add column if not exists cpu_count integer not null default 0 check (cpu_count between 0 and 9);

create or replace function public.qt_create_room_v2(p_channel integer,p_difficulty integer,p_max_players integer,p_title text,p_mode text default 'normal',p_cpu_count integer default 0)
returns uuid language plpgsql security definer set search_path=public as $$
declare rid uuid; nick text;
begin
  if p_mode not in ('normal','cpu') then raise exception 'invalid_mode'; end if;
  if p_difficulty not between 1 and 4 then raise exception 'invalid_difficulty'; end if;
  if p_max_players not between 2 and 10 then raise exception 'invalid_max_players'; end if;
  if p_cpu_count not between 0 and 9 then raise exception 'invalid_cpu_count'; end if;
  select nickname into nick from public.profiles where id=auth.uid();
  insert into public.rooms(channel,title,mode,difficulty,max_players,cpu_count,status,host_id)
  values(p_channel,left(coalesce(nullif(trim(p_title),''),'퀴즈방'),30),p_mode,p_difficulty,p_max_players,p_cpu_count,'waiting',auth.uid()) returning id into rid;
  insert into public.room_members(room_id,user_id,nickname,ready,score) values(rid,auth.uid(),coalesce(nick,'유저'),false,0);
  return rid;
end $$;
grant execute on function public.qt_create_room_v2(integer,integer,integer,text,text,integer) to authenticated;
